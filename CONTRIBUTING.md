...
The workflow pushes with a fine-grained personal access token stored as the repository secret `RENOVATE_TOKEN` (rather than `GITHUB_TOKEN`) so the resulting commit triggers the required test workflows.
